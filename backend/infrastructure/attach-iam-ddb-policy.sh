...
# attach-iam-ddb-policy.sh
# Create a managed IAM policy from iam-policy-ddb.json (if not exists)
# and attach it to the specified IAM Role (role name or ARN).
# Usage:
#   ./attach-iam-ddb-policy.sh --role ROLE_NAME_OR_ARN [--policy-name NAME]

POLICY_FILE="$(dirname "$0")/iam-policy-ddb.json"
POLICY_NAME_DEFAULT="proov-ddb-access"
