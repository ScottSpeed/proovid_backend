...
# attach-iam-sqs-worker-policy.sh
# Create a managed IAM policy from iam-policy-sqs-worker.json (if not exists)
# and attach it to the specified IAM Role (role name or ARN).
# Usage:
#   ./attach-iam-sqs-worker-policy.sh --role ROLE_NAME_OR_ARN [--policy-name NAME]

POLICY_FILE="$(dirname "$0")/iam-policy-sqs-worker.json"
POLICY_NAME_DEFAULT="proov-sqs-worker-access"
